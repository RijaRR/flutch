 ...
 * - Supporter un worker BullMQ lancé soit dans le process web, soit dans un process dédié.
 * - Garder un fallback inline si Redis n'est pas configuré pour ne pas bloquer l'app.
 *
 * Quand Redis est disponible, ce module sert à la fois de producteur (route HTTP)
 * et de consommateur (worker). Quand Redis ne l'est pas, on garde un comportement
 * compatible avec l'existant via un fallback inline lancé après la réponse HTTP.
 */
 ...
 *
 * Ce point d'entrée peut être utilisé depuis le serveur web ou depuis un process
 * dédié (`node workers/webhookWorker.js`). L'appelant décide s'il veut consommer
 * les jobs localement ou seulement les produire.
 */
