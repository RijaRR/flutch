...
router.post('/pipedrive', (req, res) => {
  ...
  // Le serveur HTTP se limite maintenant à accuser réception puis à pousser un job.
  // Le vrai traitement métier est exécuté par le worker BullMQ quand Redis est actif.
  enqueueWebhookJob({ event, current })
    .then(({ fallback, jobId }) => {
